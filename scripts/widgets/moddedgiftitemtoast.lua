...
local ModdedGiftItemToast = Class(Widget, function(self, owner)
    Widget._ctor(self, "ModdedGiftItemToast")

    self.owner = owner
    self.root = self:AddChild(Widget("ROOT"))

    self.tab_gift = self.root:AddChild(UIAnimButton("tab_gift", "tab_gift", nil, nil, "off", nil, nil))
	self.tab_gift.animstate:OverrideSymbol("GIFT", "tab_moddedgift", "GIFT")
    self.tab_gift:Disable()

    self.tab_gift:SetTooltip(STRINGS.UI.ITEM_SCREEN.DISABLED_MODDEDTOAST_TOOLTIP)
    self.tab_gift:SetTooltipPos(0, -40, 0)

    self.inst:ListenForEvent("moddedgiftreceiverupdate", function(player, data)
...
function ModdedGiftItemToast:UpdateElements()
...
function ModdedGiftItemToast:ToggleHUDFocus(focus)
    self.hud_focus = focus
    self:UpdateControllerHelp()
end

function ModdedGiftItemToast:ToggleController(hide)
    self.controller_hide = hide
    self:UpdateElements()
end

function ModdedGiftItemToast:ToggleCrafting(hide)
    self.craft_hide = hide
    self:UpdateElements()
end

function ModdedGiftItemToast:OnToast(num)
    if num == 0 then
        self.tab_gift:Disable()
    end
...
function ModdedGiftItemToast:CheckControl(control, down)
...
function ModdedGiftItemToast:DoOpenGift()
    if not self.owner:HasTag("busy") then
        local time = GetTime()
        if time - last_click_time > TIMEOUT then
            last_click_time = time
            if not TheWorld.ismastersim then
				SendModRPCToServer(GetModRPC("ModdedSkins", "OpenModdedGift"))
            elseif self.owner.components.moddedgiftreceiver ~= nil then
                self.owner.components.moddedgiftreceiver:OpenNextGift()
            end
        end
    end
end

function ModdedGiftItemToast:EnableClick()
...
function ModdedGiftItemToast:OnClickEnabled()
...
function ModdedGiftItemToast:DisableClick()
    self.tab_gift:Disable()
    self.tab_gift:SetTooltip(STRINGS.UI.ITEM_SCREEN.DISABLED_MODDEDTOAST_TOOLTIP)
    self.enabled = false
    self:UpdateControllerHelp()
...
function ModdedGiftItemToast:UpdateControllerHelp()
...
function ModdedGiftItemToast:OnUpdate()
